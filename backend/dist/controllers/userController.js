...
async function getPublicProfile(req, res, next) {
  try {
    ...
    const totalTests = await database_1.default.testAttempt.count({
      where: {
        userId: user.id,
        completedAt: { not: null },
      },
    });
    const testAttempts = await database_1.default.testAttempt.findMany({
      where: {
        userId: user.id,
        completedAt: { not: null },
      },
    ...
    const averageScore =
      totalTests > 0 ? testAttempts.reduce((sum, a) => sum + (a.score || 0), 0) / totalTests : 0;
...
async function updateProfile(req, res, next) {
  try {
    const userId = req.userId;
    const { name, status, currentGrade, desiredDirection, motivation, avatar, bio, isPublic } =
      req.body;
...
async function updateAvatar(req, res, next) {
  try {
    ...
    if (!avatar) {
      throw new errorHandler_1.AppError('Аватар не указан', 400);
    }
    const user = await database_1.default.user.update({
      where: { id: userId },
      data: { avatar },
...
async function updatePrivacy(req, res, next) {
  try {
    ...
    if (typeof isPublic !== 'boolean') {
      throw new errorHandler_1.AppError('Некорректное значение приватности', 400);
    }
    const user = await database_1.default.user.update({
      where: { id: userId },
      data: { isPublic },
  ...
  try {
    ...
    testAttempts.forEach(attempt => {
      ...
      if (
        ...
        (attempt.completedAt && attempt.completedAt > statsBySubject[subject].lastAttemptDate)
      ) {
    ...
    const timeHeatmapArray = Object.keys(timeHeatmap).map(hour => ({
      ...
      avgScore:
        timeHeatmap[parseInt(hour)].count > 0
          ? Math.round(
              (timeHeatmap[parseInt(hour)].totalScore / timeHeatmap[parseInt(hour)].count) * 10
            ) / 10
          : 0,
    }));
    // Calculate platform average
    const allAttempts = await database_1.default.testAttempt.findMany({
    ...
    const platformAverage =
      allAttempts.length > 0
        ? Math.round(
            (allAttempts.reduce((sum, a) => sum + (a.score || 0), 0) / allAttempts.length) * 10
          ) / 10
        : 0;
    ...
    let maxAttempts = 0;
    Object.entries(statsBySubject).forEach(([subject, stats]) => {
    ...
    const detailedAttempts = await database_1.default.testAttempt.findMany({
      where: {
        userId,
        completedAt: { not: null },
      },
    ...
    const allUsersStats = await database_1.default.user.findMany({
      where: {
        isPublic: true,
        testAttempts: {
          some: {
    ...
    const percentile =
      userAverages.length > 1 ? Math.round((usersBelow / (userAverages.length - 1)) * 100) : 50;
...
async function getAchievements(req, res, next) {
  try {
    ...
    const achievements = await achievementService_1.default.getAllAchievementsWithProgress(userId);
    const stats = await achievementService_1.default.getUserAchievementStats(userId);
    res.json({
      achievements,
      stats,
    });
...
async function checkAchievements(req, res, next) {
  try {
    ...
    const newAchievements = await achievementService_1.default.checkAndUnlockAchievements(userId);
    res.json({
...
async function getLeaderboard(req, res, next) {
  try {
    ...
    const usersWithStats = await database_1.default.user.findMany({
      where: {
        isPublic: true,
        testAttempts: {
          some: {
    ...
    const leaderboard = usersWithStats.map(user => {
      ...
      const points = Math.round(
        totalTests * 10 + averageScore * 5 + bestScore * 2 + achievementsCount * 50
      );
      return {
        id: user.id,
        username: user.username,
    ...
    const rankedLeaderboard = leaderboard.slice(0, parseInt(limit)).map((user, index) => ({
      ...user,
      rank: index + 1,
    }));
    res.json({
  ...
  try {
    ...
    const userAvgScore =
      userTests > 0
        ...
        : 0;
    const userBestScore =
      userTests > 0 ? Math.max(...currentUser.testAttempts.map(a => a.score || 0)) : 0;
    const userAchievements = currentUser.userAchievements.length;
    const userPoints = Math.round(
      userTests * 10 + userAvgScore * 5 + userBestScore * 2 + userAchievements * 50
    );
    const higherRankedCount = await database_1.default.$queryRaw`
