...
class AntiCheatService {
    ...
    analyzeAnswerTimes(answerTimes) {
        ...
        const averageTimeMs = answerTimes.length > 0
            ? answerTimes.reduce((a, b) => a + b, 0) / answerTimes.length
            : 0;
        if (fastAnswersCount >= SUSPICIOUS_FAST_ANSWERS_THRESHOLD) {
            reasons.push(`${fastAnswersCount} ответов даны слишком быстро (менее ${MIN_ANSWER_TIME_MS / 1000} сек)`);
        }
    ...
    async submitTestWithAntiCheat(userId, testId, answers, questionsOrder) {
        ...
        for (const answer of answers) {
            ...
            if (Array.isArray(answer.answer) && Array.isArray(correctAnswer)) {
                const isCorrect = answer.answer.length === correctAnswer.length &&
                    answer.answer.every((a) => correctAnswer.includes(a));
                if (isCorrect)
                    correctCount++;
            }
            else if (String(answer.answer).toLowerCase().trim() ===
