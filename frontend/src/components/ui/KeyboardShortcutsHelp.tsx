  ...
  return (
    <>
      <motion.button
        ...
        className={`flex items-center gap-2 px-3 py-2 bg-slate-100 hover:bg-slate-200 rounded-lg text-slate-500 hover:text-slate-900 transition-colors ${className}`}
              ...
              <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden">
                <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200">
                  <div className="flex items-center gap-3">
                    ...
                    <h2 className="text-lg font-semibold text-slate-900">Горячие клавиши</h2>
                  </div>
                  <button
                    onClick={() => setIsOpen(false)}
                      ...
                      <div className="flex items-center gap-1">
                        {shortcut.keys.map((key, keyIndex) => (
                          <span key={keyIndex} className="flex items-center gap-1">
                            <kbd className="px-2 py-1 bg-slate-100 rounded text-sm font-mono text-slate-700 border border-slate-200">
                              {key}
                            </kbd>
                ...
                <div className="px-6 py-4 bg-slate-50 border-t border-slate-200">
                  <p className="text-sm text-slate-500 text-center">
                    Нажмите <kbd className="px-1.5 py-0.5 bg-slate-200 rounded text-xs">?</kbd> для
...
export function KeyboardShortcutsHint({ className = '' }: { className?: string }) {
  return (
    <div className={`flex items-center gap-4 text-sm text-slate-500 ${className}`}>
      ...
      <span className="flex items-center gap-1">
        <kbd className="px-1.5 py-0.5 bg-slate-100 rounded text-xs">1-5</kbd>
