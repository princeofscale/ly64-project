...
const subjectDescriptions: Record<string, string> = {
 ...
 CHEMISTRY: 'Проверьте знания по химии',
 HISTORY: 'Проверьте знание истории',
 ENGLISH: 'Проверьте уровень английского языка',
 SOCIAL: 'Право, экономика, политика и общество',
 LITERATURE: 'Анализ произведений, авторы и термины',
};
...
export default function TestSetupPage() {
 ...
 void navigate('/test/variants', {
 state: { grade: selectedGrade, subject, level: selectedLevel, examMode },
 });
 };
 ...
 <div className="mb-8">
 <h2 className="text-2xl font-semibold  mb-4 flex items-center" style={{ color: 'var(--color-text)' }}>
 ...
 examMode ? 'bg-red-100' : 'bg-slate-100'
 }`}>
 <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className={`w-5 h-5 ${examMode ? 'text-red-600' : 'text-slate-500'}`}>
 <path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z" />
 </svg>
 </div>
 <div>
 <div className={`font-semibold ${examMode ? 'text-red-700' : 'text-slate-900'}`}>
 Режим экзамена
 </div>
 <div className={`text-sm ${examMode ? 'text-red-600' : 'text-slate-500'}`}>
 Имитация реального экзамена
 </div>
 </div>
 </div>
 <div className={`w-12 h-7 rounded-full transition-all relative ${
 examMode ? 'bg-red-500' : 'bg-slate-300'
 }`}>
 <div className={`w-5 h-5 bg-white rounded-full absolute top-1 transition-all shadow-sm ${
 examMode ? 'right-1' : 'left-1'
 }`} />
 </div>
 </div>
 {examMode && (
 <div className="mt-3 pl-13 text-sm text-red-700 space-y-1">
 <p>• Нельзя вернуться к предыдущим заданиям</p>
 <p>• Полноэкранный режим</p>
 <p>• Отслеживание переключения вкладок</p>
 <p>• Автосдача при истечении времени</p>
 </div>
 )}
 </button>
 </div>
 ...
 <Button variant="outline" onClick={() => { void navigate('/dashboard'); }} className="flex-1">
 Отмена
 </Button>
