...
import { vprHistory8Variants, VprHistory8Variant, VprHistory8Task } from '../data/vpr-history8-variants';
...
export default function VprHistory8TestPage() {
  const navigate = useNavigate();
  const location = useLocation();
  const { grade } = location.state || {};

  const [phase, setPhase] = useState<TestPhase>('select');
  const [selectedVariant, setSelectedVariant] = useState<VprHistory8Variant | null>(null);
  ...
  const handleSelectVariant = (variant: VprHistory8Variant) => {
  ...
  const calculateScore = useCallback(() => {
    ...
    selectedVariant.tasks.forEach((task) => {
      ...
      if (task.type !== 'detailed' && task.type !== 'map_work' && answers[task.number]) {
  ...
  const getTimeColor = () => {
    if (!selectedVariant) return 'text-amber-400';
    const totalTime = selectedVariant.duration * 60;
    if (timeLeft > totalTime * 0.5) return 'text-amber-400';
  ...
  if (phase === 'select') {
    return (
      <div className="min-h-screen bg-gray-950 relative overflow-hidden py-12 px-4">
        ...
        <div className="absolute top-20 right-10 w-96 h-96 bg-amber-500/20 rounded-full blur-[120px]" />
        <div className="absolute bottom-20 left-10 w-96 h-96 bg-orange-500/20 rounded-full blur-[120px]" />

        <div className="relative z-10 max-w-4xl mx-auto">
          <button
            onClick={() => navigate('/dashboard')}
            className="group mb-6 flex items-center gap-2 text-gray-400 hover:text-amber-400 transition-colors"
          >
          ...
          <div className="text-center mb-10">
            ...
            <h1 className="text-4xl font-bold bg-gradient-to-r from-amber-400 via-orange-400 to-red-400 bg-clip-text text-transparent mb-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {vprHistory8Variants.map((variant, index) => (
              <button
                key={variant.id}
                onClick={() => handleSelectVariant(variant)}
                className="group bg-gradient-to-br from-gray-900/90 to-gray-800/90 backdrop-blur-xl border border-amber-500/50 hover:border-amber-400 rounded-2xl p-6 text-left transition-all duration-300 hover:scale-[1.02] hover:shadow-[0_0_30px_rgba(251,191,36,0.3)] animate-scale-in"
                style={{ animationDelay: `${index * 50}ms` }}
              >
                <div className="flex items-center justify-between mb-4">
                  <span className="text-3xl font-bold text-amber-400">#{variant.id}</span>
          ...
          <div className="mt-8 p-6 bg-gradient-to-r from-amber-500/10 to-orange-500/10 border border-amber-500/30 rounded-2xl">
            <h3 className="text-lg font-semibold text-white mb-3 flex items-center gap-2">
              <span>ℹ️</span> Особенности ВПР по истории для 8 класса
            </h3>
            <ul className="space-y-2 text-gray-300 text-sm">
              <li>• <strong>10 заданий:</strong> разнообразные типы вопросов</li>
              <li>• <strong>Период:</strong> Россия XVIII века - эпоха Петра I, дворцовые перевороты, Екатерина II</li>
              <li>• <strong>Время:</strong> 1 час 30 минут (90 минут)</li>
              <li>• <strong>Форматы:</strong> хронология, карты, исторические источники, культура, аргументация</li>
              <li>• <strong>Важно:</strong> имена правителей писать словами (Пётр Первый, а не Пётр I)</li>
            </ul>
          </div>
        </div>
      </div>
      ...
      <div className="min-h-screen bg-gray-950 relative overflow-hidden py-12 px-4">
        ...
        <div className="relative z-10 max-w-3xl mx-auto">
          <div className="bg-gradient-to-br from-gray-900/90 to-gray-800/90 backdrop-blur-xl border border-gray-700/50 rounded-3xl p-8 text-center animate-scale-in">
            ...
            <span className="px-4 py-2 bg-amber-500/20 text-amber-400 rounded-xl text-sm border border-amber-500/30 inline-block">
              ВПР История 8 класс
            </span>

            <div className="mt-6 mb-8 p-6 bg-gray-800/50 rounded-2xl">
              <div className="text-5xl font-bold bg-gradient-to-r from-amber-400 via-orange-400 to-red-400 bg-clip-text text-transparent mb-2">
                {score}%
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 text-center">
                <div>
                  <div className="text-2xl font-bold text-amber-400">{answeredCount}/{selectedVariant.tasks.length}</div>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-6">
              ...
              <div className="text-sm text-gray-400">
                <span className="text-amber-400 font-semibold">{answeredCount}</span> / {selectedVariant.tasks.length}
              </div>
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-4 py-8">
        <div className="bg-gradient-to-br from-gray-900/90 to-gray-800/90 backdrop-blur-xl border border-gray-700/50 rounded-3xl p-8 animate-fade-in">
          <div className="flex items-start justify-between mb-6">
            <div>
              <div className="flex items-center gap-3 mb-2">
                <span className="px-3 py-1 bg-gradient-to-r from-amber-500/20 to-orange-500/20 border border-amber-500/30 rounded-xl text-amber-400 font-semibold">
          ...
          <div className="mb-8 p-6 bg-gray-800/50 rounded-2xl border border-gray-700/50">
            ...
            {currentTask.additionalText && (
              <div className="mt-4 p-3 bg-blue-500/10 border border-blue-500/30 rounded-xl">
                <p className="text-sm text-blue-400">
                  ℹ️ {currentTask.additionalText}
                </p>
              </div>
            )}
            {currentTask.hint && (
          ...
          <div className="mb-8">
                  ...
                  <button
                    ...
                    className={`w-full p-4 rounded-xl border-2 text-left transition-all duration-200 ${
                      answers[currentTask.number] === option
                        ...
                        : 'border-gray-700 bg-gray-800/30 text-gray-300 hover:border-amber-500/50'
                    }`}
                  >
                    {option}
                  </button>
            ...
            {currentTask.type === 'multiple_choice' && currentTask.options && (
              <div className="space-y-3">
                <p className="text-sm text-gray-400 mb-2">Выберите два правильных ответа</p>
                {currentTask.options.map((option, index) => {
                      ...
                      onClick={() => {
                        ...
                        if (isSelected) {
                          ...
                          if (currentAnswers.length >= 2) {
                            toast.error('Можно выбрать только 2 варианта');
                            return;
                          }
            ...
            {(currentTask.type === 'short' || currentTask.type === 'sequence') && (
              <div>
                <label className="block text-sm text-gray-400 mb-2">
                  {currentTask.type === 'sequence' ? 'Введите последовательность цифр:' : 'Введите ответ:'}
                </label>
                <input
                  type="text"
                  value={answers[currentTask.number] || ''}
                  onChange={(e) => handleAnswer(e.target.value)}
                  placeholder={currentTask.type === 'sequence' ? 'Например: 312' : 'Ваш ответ'}
                  className="w-full px-4 py-3 bg-gray-800/50 border border-gray-700 rounded-xl text-white placeholder-gray-500 focus:ring-2 focus:ring-amber-500 focus:border-amber-500 transition-all"
                />
                {currentTask.type === 'sequence' && (
                  <p className="mt-2 text-sm text-gray-500">
                    📝 Запишите цифры в правильном порядке без пробелов
                  </p>
                )}
              </div>
            )}

            {(currentTask.type === 'detailed' || currentTask.type === 'map_work') && (
              <div>
                <label className="block text-sm text-gray-400 mb-2">
                  Развёрнутый ответ {currentTask.type === 'map_work' && '(работа с картой)'}:
                </label>
                <textarea
                  value={answers[currentTask.number] || ''}
                  onChange={(e) => handleAnswer(e.target.value)}
                  placeholder="Напишите ваш ответ..."
                  rows={currentTask.type === 'map_work' ? 6 : 10}
                  className="w-full px-4 py-3 bg-gray-800/50 border border-gray-700 rounded-xl text-white placeholder-gray-500 focus:ring-2 focus:ring-amber-500 focus:border-amber-500 transition-all resize-vertical"
                />
                <p className="mt-2 text-sm text-gray-500">
                  {currentTask.type === 'map_work'
                    ? '🗺️ Опишите расположение объектов на карте'
                    : '📜 Обоснуйте свой ответ, опираясь на исторические факты'}
                </p>
              </div>
            )}
          </div>
              ...
              <button
                ...
                className={`aspect-square rounded-lg font-semibold text-sm transition-all ${
                  currentTaskIndex === index
                    ...
                    : 'bg-gray-800/50 border border-gray-700 text-gray-400 hover:border-amber-500/50'
                }`}
              >
                {task.number}
              </button>
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-gray-800 rounded-2xl p-6 max-w-md w-full border border-gray-700 animate-scale-in">
            ...
            <p className="text-gray-400 mb-2">
              Вы ответили на <span className="text-amber-400 font-semibold">{answeredCount}</span> из <span className="text-white">{selectedVariant.tasks.length}</span> вопросов.
            </p>
            ...
            <div className="flex flex-col gap-3">
              <button
                ...
                className="w-full px-4 py-3 bg-amber-500 text-white font-medium rounded-xl hover:bg-amber-600 transition-all"
              >
                Продолжить тест
              </button>
              <button
                onClick={handleFinish}
                className="w-full px-4 py-3 bg-orange-600 text-white font-medium rounded-xl hover:bg-orange-700 transition-all"
              >
