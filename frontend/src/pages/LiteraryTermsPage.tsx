...
export default function LiteraryTermsPage() {
 ...
 return LITERARY_TERMS.filter(t => {
 ...
 <h1 className="text-3xl font-bold  mb-1" style={{ color: 'var(--color-text)' }}>Словарь литературных терминов</h1>
 <p className="text-slate-500">
 Тропы, фигуры, жанры, стихосложение · {LITERARY_TERMS.length} терминов
 </p>
 </div>
 ...
 className="w-full bg-white border border-slate-200 rounded-xl pl-10 pr-10 py-3 text-slate-900 placeholder-slate-400 focus:outline-none focus:border-violet-500 focus:ring-1 focus:ring-violet-500 shadow-sm"
 />
 {search && (
 <button
 onClick={() => setSearch('')}
 ...
 const count = LITERARY_TERMS.filter(t => t.category === cat.key).length;
 return (
 <button
 key={cat.key}
 ...
 {terms.map(term => {
 const colors = CAT_COLOR[term.category] ?? CAT_COLOR['theory']!;
 const cat = LITERARY_CATEGORIES.find(c => c.key === term.category);
 return (
 <div
 key={term.id}
