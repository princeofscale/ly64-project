    ...
    const checkNewAchievements = async () => {
      try {
        ...
        if (!hasInitialized.current) {
          previousAchievements.current = new Set(unlockedAchievements.map(a => a.id));
...
const showAchievementToast = (achievement: Achievement) => {
  ...
  toast.custom(
    t => (
      <div
        className={`${t.visible ? animationClass : 'animate-leave'} max-w-md w-full bg-gradient-to-r from-yellow-400 via-yellow-500 to-yellow-600 shadow-2xl rounded-2xl pointer-events-auto flex ring-1 ring-black ring-opacity-5 p-4`}
      >
        <div className="flex-1 flex items-center">
          ...
          <div className="ml-4 flex-1">
            ...
            <p className="mt-1 text-sm text-yellow-100">{achievement.description}</p>
