...
export default function ProfilePage() {
  ...
  const handleSave = async () => {
    ...
    if (!/^[a-z0-9_]{3,32}$/.test(trimmedUsername)) {
      setUsernameError('3–32 символа: латиница, цифры и _');
      return;
    }
    ...
    try {
      ...
      const response = await fetch('/api/users/profile', {
        ...
        body: JSON.stringify({ ...editForm, username: trimmedUsername }),
      });

      const data = await response.json();

      if (!response.ok) {
        const msg: string = data?.message || 'Ошибка сохранения';
        if (msg.toLowerCase().includes('юзернейм') || msg.toLowerCase().includes('занят')) {
          ...
          <div className="px-8 pb-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4">
              ...
              <div className="bg-gradient-to-br from-blue-50 to-cyan-50 border border-blue-200 rounded-xl p-4 text-center">
                ...
                <p className="text-xs text-slate-500">Минут занятий</p>
              </div>
              <div className="bg-gradient-to-br from-pink-50 to-purple-50 border border-pink-200 rounded-xl p-4 text-center">
                <div className="flex items-center justify-center gap-2 mb-1">
                  <span className="text-2xl">📈</span>
              ...
              <div className="bg-gradient-to-br from-green-50 to-emerald-50 border border-green-200 rounded-xl p-4 text-center">
                ...
                <p className="text-xs text-slate-500">Любимый предмет</p>
              </div>
            </div>
          </div>
        </div>
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-1 text-xs text-slate-500">
                ...
                {[0, 1, 2, 3, 4].map(i => (
                  <div
                    key={i}
                    className={`w-2 h-2 rounded ${['bg-slate-100', 'bg-cyan-100', 'bg-cyan-300', 'bg-cyan-500', 'bg-cyan-600'][i]}`}
                  />
                ))}
                <span>Больше</span>
              </div>
              ...
              {Array.from({ length: 24 }, (_, hour) => {
                ...
                return (
                  <div key={hour} className="text-center">
                    ...
                    >
                      <span className={`text-xs font-mono ${intensity === 0 ? 'text-slate-400' : 'text-slate-700'}`}>{hour}</span>
                      <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover:block bg-slate-900 text-white text-xs rounded-lg px-3 py-2 whitespace-nowrap z-10 shadow-xl">
                        <div className="font-bold">
                          {hour}:00 - {hour + 1}:00
                        </div>
            ...
            <div className="flex items-center justify-between mt-6 text-xs text-slate-500">
              ...
              <div className="flex gap-1">
                {[0, 1, 2, 3, 4].map(i => (
                  <div
                    key={i}
                    className={`w-5 h-5 rounded ${['bg-slate-100', 'bg-red-200', 'bg-yellow-300', 'bg-green-400', 'bg-green-500'][i]}`}
                  />
                ))}
              </div>
            ...
            {(stats?.percentile || 0) >= 70 ? (
              <div className="mt-6 p-4 bg-green-50 border border-green-200 rounded-xl">
                <div className="flex items-start gap-3">
                  <span className="text-2xl">🏆</span>
                  <div>
                    <p className="text-green-700 font-semibold mb-1">Вы в топе!</p>
                    <p className="text-slate-600 text-sm">
              ...
              <div className="mt-6 p-4 bg-cyan-50 border border-cyan-200 rounded-xl">
                <div className="flex items-start gap-3">
                  <span className="text-2xl">📈</span>
                  <div>
                    <p className="text-cyan-700 font-semibold mb-1">Хороший результат!</p>
                    <p className="text-slate-600 text-sm">
              ...
              <div className="mt-6 p-4 bg-amber-50 border border-amber-200 rounded-xl">
                <div className="flex items-start gap-3">
                  <span className="text-2xl">💪</span>
                  <div>
                    <p className="text-amber-700 font-semibold mb-1">Время для рывка!</p>
                    <p className="text-slate-600 text-sm">
            ...
            <div className="bg-white border border-slate-200 shadow-sm rounded-2xl p-6">
              ...
              {stats?.predictionFactors && stats.predictionFactors.length > 0 && (
                <div className="flex flex-wrap gap-2 mb-4">
                  {stats.predictionFactors.map((factor, i) => (
                    <span
                      key={i}
                      className="px-3 py-1 bg-pink-50 border border-pink-200 rounded-full text-pink-700 text-xs"
                    >
                      {factor}
                    </span>
                  ))}
              ...
              <div className="p-4 bg-purple-50 border border-purple-200 rounded-xl">
                <div className="flex items-start gap-3">
                  <span className="text-2xl">✨</span>
                  <div>
                    <p className="text-purple-700 font-semibold mb-1">Анализ ИИ</p>
                    <p className="text-slate-600 text-sm">
              ...
              {stats?.weakTopics && stats.weakTopics.length > 0 && (
                <div className="p-4 bg-orange-50 border border-orange-200 rounded-xl">
                  <div className="flex items-start gap-3">
                    <span className="text-2xl">📚</span>
                    <div>
                      <p className="text-orange-700 font-semibold mb-1">Рекомендация</p>
                      <p className="text-slate-600 text-sm">
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-gray-800 rounded-2xl p-6 max-w-md w-full border border-gray-700">
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-sm text-gray-400 mb-2">Юзернейм</label>
                <div
                  className={`flex items-center bg-gray-700 border rounded-xl focus-within:border-cyan-500 transition-colors ${
                ...
                {usernameError ? (
                  ...
                  <p className="text-xs text-gray-500 mt-1.5">
                    3–32 символа: латиница, цифры и _. Ссылка: /profiles/{editForm.username || 'username'}
                  </p>
                )}
              </div>
            ...
            <div className="flex gap-3 mt-6">
              <button
                ...
                className="flex-1 py-3 bg-gray-700 text-white rounded-xl hover:bg-gray-600 transition-colors disabled:opacity-50"
              >
                Отмена
              </button>
              <button
