...
export function getRandomGreeting(): string {
  return greetings[Math.floor(Math.random() * greetings.length)] ?? 'Привет';
}

export function getRandomMotivation(): string {
  return motivationalPhrases[Math.floor(Math.random() * motivationalPhrases.length)] ?? 'Продолжай в том же духе';
}
