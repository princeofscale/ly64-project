...
export const checkAccountLockout = async (email: string): Promise<{ locked: boolean; remainingTime?: number }> => {
...
export const recordFailedLogin = async (email: string): Promise<{ locked: boolean; attempts: number }> => {
  const key = LOCKOUT_KEY_PREFIX + email.toLowerCase();
  try {
    const raw = await redis.get(key);
    const entry: LockoutEntry = raw ? JSON.parse(raw) : { attempts: 0, lockedUntil: null };
    ...
    await redis.set(key, JSON.stringify(entry), 'EX', LOCKOUT_TTL_SECONDS);
...
export const clearFailedLogins = async (email: string): Promise<void> => {
  try {
    ...
    // Redis unavailable — lockout state is already ephemeral
  }
};
