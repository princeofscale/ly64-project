...
export class AccountLockoutService {
  public async checkLockout(email: string): Promise<LockoutStatus> {
    return checkLockout(email);
  }

  public async recordFailedAttempt(email: string): Promise<FailedLoginAttempt> {
    return recordFailed(email);
  }

  public async clearAttempts(email: string): Promise<void> {
