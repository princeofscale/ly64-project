  ...
  public async updateProfile(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      if (updateData.username !== undefined) {
        const normalized = updateData.username.trim().toLowerCase();
        if (!/^[a-z0-9_]{3,32}$/.test(normalized)) {
          throw new AppError(ERROR_MESSAGES.USERNAME_INVALID, HTTP_STATUS.BAD_REQUEST);
        }
        ...
        if (existing && existing.id !== userId) {
          throw new AppError(ERROR_MESSAGES.USERNAME_TAKEN, HTTP_STATUS.BAD_REQUEST);
        }
        (updateData as Record<string, unknown>).username = normalized;
      }
