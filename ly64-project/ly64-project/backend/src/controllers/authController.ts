...
export class AuthController {
  ...
  public async login(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      await this.checkAccountLockout(validatedData.email, req);
  ...
  private async checkAccountLockout(email: string, request: Request): Promise<void> {
    const lockoutStatus = await accountLockoutService.checkLockout(email);
  ...
  private async executeLogin(data: LoginInput, request: Request): Promise<AuthResult> {
    try {
      ...
      await accountLockoutService.clearAttempts(data.email);
      accountLockoutService.logSuccessfulLogin(result.user.id, result.user.email, request);

      return result;
    } catch (error) {
      await this.handleLoginFailure(data.email, request, error);
      throw error;
    }
  }

  private async handleLoginFailure(email: string, request: Request, error: unknown): Promise<void> {
    const errorMessage = RequestUtils.extractErrorMessage(error);
    const failedAttempt = await accountLockoutService.recordFailedAttempt(email);
