...
import type { RocketLeaderboardEntry } from '@lyceum64/shared';
...
type Mode = 'study' | 'rocket';
type RocketPeriod = 'all' | 'week' | 'day';
type Subject =
  | 'all'
  | 'MATHEMATICS'
  | 'PHYSICS'
...
export default function LeaderboardPage() {
  ...
  const [rocketPeriod, setRocketPeriod] = useState<RocketPeriod>('all');
  const [rocketBoard, setRocketBoard] = useState<RocketLeaderboardEntry[]>([]);
  ...
  useEffect(() => {
    if (mode === 'study') {
      ...
      loadRocketLeaderboard();
    }
  }, [period, subject, token, mode, rocketPeriod]);

  const loadRocketLeaderboard = async () => {
    try {
      setRocketLoading(true);
      const data = await rocketApi.getLeaderboard(rocketPeriod, 50);
      setRocketBoard(data);
    } catch (error) {
      console.error('Failed to load rocket leaderboard:', error);
    } finally {
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-4 py-8">
        <div className="mb-8">
          ...
          <div className="flex flex-col md:flex-row md:flex-items-center md:justify-between gap-4">
            ...
            {mode === 'study' ? (
              ...
              <div className="flex gap-3">
                <select
                  value={rocketPeriod}
                  onChange={e => setRocketPeriod(e.target.value as RocketPeriod)}
                  className="bg-slate-50 border border-slate-200 rounded-xl px-4 py-2 text-slate-900 focus:outline-none focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20 transition-colors"
                >
          ...
          <div className="mt-6 inline-flex rounded-xl bg-slate-100 p-1">
            ...
            >
              <Trophy className="w-4 h-4" /> Учёба
            </button>
            <button
            ...
            >
              <Rocket className="w-4 h-4" /> Ракетка
            </button>
          </div>
        </div>

        {mode === 'study' && userRank && currentUser && (
        ...
        {mode === 'study' &&
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-12 text-center shadow-lg">
            ...
            <h3 className="text-xl font-bold text-slate-900 mb-2">Пока нет данных</h3>
        ...
        {mode === 'rocket' && (
          ...
          ) : rocketBoard.length === 0 ? (
            <div className="bg-white border border-slate-200 rounded-2xl p-12 text-center shadow-lg">
              <div className="text-5xl mb-4">🚀</div>
              <h3 className="text-xl font-bold text-slate-900 mb-2">Ещё никто не играл</h3>
              <p className="text-slate-600 mb-4">
                Будь первым, кто попадёт в рейтинг Ракетки!
              </p>
              <Link
                to="/rocket"
                className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-gradient-to-r from-amber-500 to-orange-500 text-white font-semibold hover:shadow-lg transition-shadow"
              >
                <Rocket className="w-4 h-4" /> Играть
              </Link>
            </div>
          ) : (
            <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-xl">
              <div className="grid grid-cols-12 gap-4 px-6 py-4 border-b border-slate-200 text-sm text-slate-600 font-medium">
                ...
                <div className="col-span-2 text-center">Ставок</div>
                <div className="col-span-2 text-center">Прибыль</div>
                <div className="col-span-2 text-center">Выиграно</div>
              </div>
              <div className="p-4 space-y-3">
                {rocketBoard.map(entry => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1 flex items-center gap-4 ml-4">
                        <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-amber-500 to-orange-500 overflow-hidden border-2 border-white shadow-md">
                          {entry.avatar ? (
                            <img
                              src={entry.avatar}
                              alt={entry.name}
                      ...
                      <div className="w-20 text-center">
                        <div className="text-slate-900 font-medium">{entry.bets}</div>
                        <div className="text-xs text-slate-500">ставок</div>
                      </div>
                      <div className="w-24 text-center">
                        <div
                          className={`font-bold ${
                            entry.netProfit >= 0 ? 'text-emerald-600' : 'text-rose-600'
                          }`}
                        >
                          {entry.netProfit >= 0 ? '+' : ''}
                          {entry.netProfit.toFixed(2)}
                        </div>
                        <div className="text-xs text-slate-500">₽ₘ</div>
                      </div>
                      <div className="w-24 text-center">
                        <div className="text-xl font-bold text-amber-600">
        ...
        {mode === 'study' && (
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-4">
              ...
              <div className="text-slate-600">за каждое достижение</div>
            </div>
          </div>
        </div>
        )}
